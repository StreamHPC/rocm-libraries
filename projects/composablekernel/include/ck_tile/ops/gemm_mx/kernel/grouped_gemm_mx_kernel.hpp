...
template <typename ScaleM    = MXScalePointer<e8m0_t, 1, 32>,
          typename ScaleN    = MXScalePointer<e8m0_t, 1, 32>,
...
struct GroupedMXGemmKernel
    : UniversalGemmKernel<TilePartitioner_, MXGemmPipeline_, EpiloguePipeline_>
{
    ...
    using Kernel = GroupedMXGemmKernel<TilePartitioner, MXGemmPipeline, EpiloguePipeline>;
    ...
    {
        // clang-format off
        return concat('_', "grouped_mx_gemm", gemm_prec_str<ADataType, BDataType>, MXGemmPipeline::GetName());
        // clang-format on
    }

    // CK_TILE_HOST static auto
    // GetWorkSpaceSize(const std::vector<KernelArgs<>>& gemm_descs) -> std::size_t
    // {
    //     return gemm_descs.size() * sizeof(KernelArgs<>);
    // }

    // CK_TILE_HOST static auto GetWorkSpaceSize(index_t group_count) -> std::size_t
    // {
    //     return group_count * sizeof(KernelArgs<>);
    // }
    ...
    CK_TILE_HOST static auto MaxOccupancyGridSize(const stream_config& s) -> dim3
    {
        ...
        HIP_CHECK_ERROR(hipOccupancyMaxActiveBlocksPerMultiprocessor(
            &occupancy, kernel, MXGemmPipeline::BlockSize, 0));
        const int grid_size = get_available_compute_units(s) * occupancy;
        return dim3(grid_size, 1, 1);
    }

    template <typename ScaleM, typename ScaleN>
    CK_TILE_HOST static constexpr auto GridSize(const GroupKernelArgs<ScaleM, ScaleN>& kargs)
    {
        ...
        if constexpr(UsePersistentKernel)
        {
            ...
            if(hipOccupancyMaxActiveBlocksPerMultiprocessor(
                   ...
                   reinterpret_cast<void*>(
                       kentry<1, GroupedMXGemmKernel, remove_cvref_t<decltype(kargs)>>),
                   KernelBlockSize,
    ...
    template <typename ScaleM, typename ScaleN>
    CK_TILE_HOST static auto
    MakeKargs(const std::vector<GroupKernelArgs<ScaleM, ScaleN>>& gemm_descs)
        -> std::vector<KernelArgs<ScaleM, ScaleN>>
    {
        std::vector<KernelArgs<ScaleM, ScaleN>> gemm_kernel_args_;
        ...
        for(std::size_t i = 0; i < gemm_descs.size(); ++i)
        {
            ...
            auto cicc = gemm_descs[i];
            gemm_kernel_args_.emplace_back(KernelArgs<ScaleM, ScaleN>(
                /*gemm_descs[i]*/ std::move(cicc), block_start, block_end));
        }
    ...
    CK_TILE_DEVICE static auto MakeCBlockWindows(EDataType* e_ptr,
                                                 const GroupKernelArgs<ScaleM, ScaleN>& kargs,
                                                 const index_t i_m,
                                                 const index_t i_n)
    {
    ...
    template <typename ScaleM, typename ScaleN>
    CK_TILE_DEVICE static auto MakeScaleABlockWindows(const GroupKernelArgs<ScaleM, ScaleN>& kargs,
                                                      const index_t i_m)
    {
    ...
    template <typename ScaleM, typename ScaleN>
    CK_TILE_DEVICE static auto MakeScaleBBlockWindows(const GroupKernelArgs<ScaleM, ScaleN>& kargs,
                                                      const index_t i_n)
    {
    ...
    template <typename ScaleM, typename ScaleN>
    CK_TILE_DEVICE void Run(const GroupKernelArgs<ScaleM, ScaleN>& kargs,
                            ...
                            const index_t block_idx_z) const
    {
        ...
        const typename Underlying::SplitKBatchOffset splitk_batch_offset(kargs, block_idx_z);
    ...
    template <typename ScaleM, typename ScaleN>
    CK_TILE_DEVICE index_t FindGroupId(const KernelArgs<ScaleM, ScaleN>* gemm_desc_ptr,
                                       index_t block_id,
    ...
    template <typename ScaleM = MXScalePointer<e8m0_t, 1, 32>, // TODO: how to pass scale types to
                                                               // the kernel
              typename ScaleN = MXScalePointer<e8m0_t, 1, 32>,
              bool U          = UsePersistentKernel,
                                   ...
                                   index_t group_count) const
    {
        ...
        const auto gemm_desc_ptr = reinterpret_cast<const KernelArgs<ScaleM, ScaleN>*>(
